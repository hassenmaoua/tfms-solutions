...
import { getActivityById } from '../../core/requests';
...
const CreateActivityModal = ({ show }) => {
  ...
  const {
    ...
    error,
    data: activityForUpdate,
  } = useQuery(
    `activity-id-${itemIdForUpdate}`,
    () => getActivityById(itemIdForUpdate),
    {
  ...
  if (!itemIdForUpdate && !activityForUpdate) {
    return (
      <Modal show={show} onClose={handleClose}>
        <Dialog.Panel className='bg-white relative shadow rounded-lg max-w-md mx-auto transition-all dark:bg-gray-800'>
          <div className=''>
            <div className='p-6 space-y-4 md:space-y-6 sm:p-8'>
              ...
              <CreateActivityForm onClose={handleClose} activity={undefined} />
            </div>
          </div>
        </Dialog.Panel>
      </Modal>
